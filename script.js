...
for (var i = 0, mathAction = ["plus", "minus", "multiplication", "divide", "percent"]; i < mathAction.length; i++) { var actionName=mathAction[i]; mathOperator=document.querySelector("._action[action=" + actionName + " ]"); mathOperator.onclick=opCallback(actionName); } equality.onclick=function (event) { buffer.push({ type: "value" , value: parseFloat(result.value, 10) }); result.value=evaluate(buffer).toString(); }; clear.onclick=function (event) { result.value="" ; while (buffer.length) { buffer.pop();} }; negate.onclick=function (event) { return result.value=-parseFloat(result.value, 10); }; point.onclick=function (event) { return result.value=parseFloat(result.value) + '.' ; };